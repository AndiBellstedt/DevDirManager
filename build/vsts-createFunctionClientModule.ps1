...
$parentModule = 'DevDirManager'
if (-not $ModuleName) { $ModuleName = 'DevDirManager.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\DevDirManager'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\DevDirManager.psd1"
$prereqHash = @{
