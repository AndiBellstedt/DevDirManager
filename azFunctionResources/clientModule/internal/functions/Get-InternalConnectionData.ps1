...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'DevDirManager.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'DevDirManager.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'DevDirManager.Client.ProtectedToken'
