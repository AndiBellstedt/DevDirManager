Describe "Restore-DevDirectory" -Tag "PublicFunction", "Restore" {
    ...
    Context "Parameter Contract" {
        ...
        Context "Parameter: DestinationPath" {
    ...
    Context "PSDrive Support" {
        It "Should handle PSDrive as DestinationPath with -WhatIf" {
            ...
            { $script:TestRepos | Restore-DevDirectory -DestinationPath $psdrivePath -WhatIf -InformationAction SilentlyContinue } | Should -Not -Throw
        }
    }

    Context "Remote Accessibility" {
        It "Should skip repositories with IsRemoteAccessible = false in WhatIf" {
            $whatIfOutput = $script:TestRepos | Restore-DevDirectory -DestinationPath $script:RestoreTarget -WhatIf -InformationVariable infoVar -WarningVariable warnVar 2>&1
